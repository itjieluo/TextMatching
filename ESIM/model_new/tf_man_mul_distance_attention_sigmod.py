...
with graph_mhd.as_default():
    ...
    true = tf.reshape(tf.cast(tf.argmax(labels, axis=-1), tf.float32), (-1,1))
    ...
    with tf.variable_scope("loss"):
        loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=true,logits=logits))
        # loss = tf.reduce_mean(tf.nn.weighted_cross_entropy_with_logits(targets=true,logits=logits,pos_weight=1.3))
